# State-space point-process filter for hippocampal position decoding.
#
...
# state-space filter whose observation model is a non-homogeneous Poisson GLM.
...
class GaussianPointProcessFilter:
    ...
    The GLM uses a quadratic spatial feature map, making the conditional
    intensity a non-homogeneous Poisson process over position.
    """
...
def main():
    ...
    spike_counts_valid = spike_counts[valid_set, :]
    ...
    print("Decoding validation set with SSPPF")
    predictions = model.predict(spike_counts_valid, y_valid[0])
    R2_valid = get_R2(y_valid, predictions)
    rho_valid = get_rho(y_valid, predictions)
    mse_valid = get_mse_by_axis(y_valid, predictions)
    nrmse_valid = get_nrmse_by_range(y_valid, predictions)
    nmse_valid = get_nmse_by_variance(y_valid, predictions)
    print("\n[SSPPF] validation R2:", R2_valid)
    print("[SSPPF] validation rho:", rho_valid)
    print("[SSPPF] validation MSE [X, Y]:", mse_valid)
    print("[SSPPF] validation NRMSE_range [X, Y]:", nrmse_valid)
    print("[SSPPF] validation NMSE_var [X, Y]:", nmse_valid)

    print("Decoding test set with SSPPF")
    test_predictions = model.predict(spike_counts_test, y_test[0])
    R2_test = get_R2(y_test, test_predictions)
    rho_test = get_rho(y_test, test_predictions)
    mse_test = get_mse_by_axis(y_test, test_predictions)
    nrmse_test = get_nrmse_by_range(y_test, test_predictions)
    nmse_test = get_nmse_by_variance(y_test, test_predictions)
    print("\n[SSPPF] test R2:", R2_test)
    print("[SSPPF] test rho:", rho_test)
    print("[SSPPF] test MSE [X, Y]:", mse_test)
    print("[SSPPF] test NRMSE_range [X, Y]:", nrmse_test)
    print("[SSPPF] test NMSE_var [X, Y]:", nmse_test)

    plot_start = min(2000, max(0, y_valid.shape[0] - 1))
    plot_end = min(5000, y_valid.shape[0])
    if plot_end <= plot_start:
        plot_start = 0
        plot_end = y_valid.shape[0]

    fig_x_ssppf = plt.figure()
    plt.plot(y_valid[plot_start:plot_end, 0] + y_train_mean[0], "b", label="validation")
    plt.plot(predictions[plot_start:plot_end, 0] + y_train_mean[0], "r", label="prediction")
    plt.xlabel("Time bin")
    plt.ylabel("X position")
    plt.legend()
    fig_x_ssppf.savefig("x_position_decoding_ssppf.jpg", dpi=300)

    fig_y_ssppf = plt.figure()
    plt.plot(y_valid[plot_start:plot_end, 1] + y_train_mean[1], "b", label="validation")
    plt.plot(predictions[plot_start:plot_end, 1] + y_train_mean[1], "r", label="prediction")
    plt.xlabel("Time bin")
    plt.ylabel("Y position")
    plt.legend()
    fig_y_ssppf.savefig("y_position_decoding_ssppf.jpg", dpi=300)
